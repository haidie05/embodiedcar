...
gesture_stable_threshold = 3  # 需要连续3帧检测到相同手势

# 命令执行帧数控制
command_frame_count = 0   # 当前命令已执行的帧数
max_command_frames = 90   # 每个命令最多执行的帧数（例如90帧，假设30fps约3秒），达到后自动停止
...
while True:
    ...
    # 如果未检测到手势，且之前有命令在执行，则发送停止命令
    if (current_gesture == GESTURE_NONE and 
        last_sent_gesture != GESTURE_NONE and 
        last_sent_gesture != GESTURE_STOP):
        send_command(GESTURE_STOP)
        print(f"No gesture detected, stopping command '{last_sent_gesture}'")
        last_sent_gesture = GESTURE_STOP
        command_frame_count = 0  # 重置计数
    
    # 发送命令：如果手势稳定且与上次发送的不同，则发送命令
    if (current_gesture != GESTURE_NONE and 
        gesture_stable_count >= gesture_stable_threshold and 
